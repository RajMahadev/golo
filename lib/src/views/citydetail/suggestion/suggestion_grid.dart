import 'package:Golo/src/views/citydetail/suggestion/suggestion_cell.dart';
...
import 'package:Golo/src/views/citydetail/Filter/filter_place_view.dart';
...
class _SuggestionGrid extends State<SuggestionGrid> {
  ...
  Widget build(BuildContext context) {
    if (selectedPriceOptions?.length == 0 &&
        ...
        selectedAmenityOptions?.length == 0) places = widget.places;
    return Container(
                    ...
                    child: Stack(
                        ...
                        children: <Widget>[
                          ...
                          Align(
                              ...
                              child: Visibility(
                                visible: (selectedPriceOptions != null &&
                                        selectedPriceOptions!.isNotEmpty) ||
                                    (selectedAmenityOptions != null &&
                                        selectedAmenityOptions!.isNotEmpty) ||
                                    (selectedTypesoptions != null &&
...
//GridView
  ...
  void filter(
    ...
    if (prices == null || types == null || amenities == null) return;
    setState(() {
      ...
      if (prices.length > 0) {
        places = places!
            .where((place) => prices.contains(place.priceRange))
            .toList();
      }
      if (types.length > 0) {
      ...
      if (amenities.length > 0) {
        places = places!.where((place) {
          List<String> tmpAmenity =
              place.amenities!.map((amenity) => "$amenity") as List<String>;
