...
import 'package:Golo/modules/controls/images/my_image_helper.dart';
...
import 'package:Golo/src/blocs/city_detail/CityDetailBloc.dart';
...
import 'package:Golo/src/providers/BlocProvider.dart';
import 'package:Golo/src/views/place_detail_overview/place_detail_overview.dart';
import 'map/city_google_map_view.dart';
import 'package:flutter/cupertino.dart';

import 'suggestion/suggestion_grid.dart';
import 'suggestion/suggestion_list.dart';
...
class _CityDetailState extends State<CityDetail> {
  ...
  Widget _tableCategories(CityDetailBloc bloc) {
    var listCat = categories
        .where((cat) => bloc.groupedPlaces.containsKey(cat.id))
        .toList();
  ...
  Widget _buildListSuggestion(CityDetailBloc bloc) {
    ...
    return Container(
        child: Column(
            children: categories
                .where((cat) => data.containsKey(cat.id))
                .toList()
  ...
  Widget _buildMap(CityDetailBloc bloc, bool isFullScreen) {
    return new GoogleMapViewCity(
      places: selectedCategoryId == null
          ? bloc.places
          : bloc.groupedPlaces[selectedCategoryId!],
      city: widget.city,
