...
class FilterCityDetail extends StatefulWidget {
  final Function(
          List<String>? prices, List<String>? types, List<String>? amenities)?
      onFilter;
  ...
  FilterCityDetail(
      {this.onFilter,
      this.selectedOptionPrices,
      this.selectedOptionAmenities,
      this.selectedOptionTypes});
...
class _FilterCityDetail extends State<FilterCityDetail> {
  ...
  void initState() {
    ...
    super.initState();
    filterSelectedOption(widget.selectedOptionPrices,
        widget.selectedOptionTypes, widget.selectedOptionAmenities);
  }
  ...
  List<Option> typesOptions = AppState()
      .placeTypes
      .map((type) =>
          Option(title: type.name, isSelected: false, key: "${type.id}"))
      .toList();
  List<Option> amenitiesOptions = AppState()
      .amenities
      .map((amenity) =>
          Option(title: amenity.name, isSelected: false, key: "${amenity.id}"))
      .toList();
  ...
  void applyFilter() {
    var prices = priceOptions
        ...
        .toList();
    var types = typesOptions
        .where((option) => option.isSelected!)
        .toList()
        .map((option) => option.key ?? "")
        .toList();
    var amenities = amenitiesOptions
        .where((option) => option.isSelected!)
        .toList()
        .map((option) => option.key ?? "")
        .toList();
    if (this.widget.onFilter != null) {
      this.widget.onFilter!(prices, types, amenities);
    }
  }

  void filterSelectedOption(List<String>? selectedPrice,
      List<String>? selectedType, List<String>? selectedAmenity) {
